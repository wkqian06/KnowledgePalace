...
# knowledge-palace — Codex runtime entry (thin adapter)
...
- Optional Codex skill metadata: `agents/openai.yaml` next to this file.

Codex dispatch:

- Project subagents are `.codex/agents/palace-<role>.toml`, each with a
  read-only sandbox and no model pin; each points to its shared contract and
  adds nothing of substance. Subagents never write files.
...
- Without native subagent support, execute each role contract in a separate
  read-only context and return its draft to the orchestrator (PROTOCOL.md,
  "Roles and dispatch").
